...
function get_cookie_bankcard() {
    ...
    if(!x[0]==''){
        var hm='';
        for(var n=0;n<x.length;n++){hm +='<li class="'+x[n].statu+'"><a href="'+x[n].url+'" class="setckout">'+x[n].name+'<i class="icoclose" data_eq="'+n+'"></i></a></li>'}
                ...
                $.ajax({
                    ...
                    success:function (data) {
                        if(data.status){
                            ...
                            var bankcardcookies = getCookie('navMsgNameOut');
                            if(bankcardcookies=='[]'){
