...
$(function () {
	...
	// 			console.log('服务器错误，请重试！')
	// 		}else {
	// 			var navs = data.bank_name;
	// 			html += '<ul class="nav childnav">';
	// 			$.each(navs, function (i, item) {
	// 				html += '<li ><a href="javascript:;" data-name="item" class="cnav'+i+'">' + item + '<b class="rightico"></b></a></li>';
	// 			});
	// 			html += '</ul>';
	// 			$('.collapsebanks').html(html);
	// 			}
	// 	},
	// 	error:function () {
	// 		console.log("服务器错误，请重试！")
    ...
    // });
    //
	// $('.collapsebanks').on('click','.childnav li [data-name="item"]',function(){
	// 	var bankname = $(this).text();
	// 	var html2 = $(this).parent().html();
	// 	console.log(html2);
	// 	var thatA = $(this).attr('class');
	// 	// return false;
	// 	var html1 = '';
	// 	$.ajax({
	// 		url:"/bankcard",
	...
	// 				console.log('服务器错误，请重试！')
	// 			}else {
	// 				var navs = data.bankcard_list;
	// 				console.log(navs);
	// 				 html1 += html2+'<ol class="third-nav" style="display: block" >';
	// 				$.each(navs, function (i,item1) {
     //               		 html1 += '<li><a href="javascript:;" data-url="' + i + '">' + item1 + '</a></li>';
     //       			});
	// 				html1 += '</ol>';
    //
	// 				console.log(html1);
	// 				console.log(typeof html1);
	// 				$('.'+thatA).parent().html(html1);
	// 				$('.'+thatA+' .rightico').addClass('caret');
	// 			}
    //
     //        }
	// 	});
	...
	// });
    //
	// $('.collapsebanks').on('click','.third-nav li a',function () {
	// 	var bankcardname = $(this).text();
	// 	$.ajax({
	// 		url:'/bankrecord',
	// 		type:'post',
	// 		dataType:'json',
...
// 获取银行卡信息
// function get_click_bankcard() {
// $('.sidebar-wrapper').on('click','.first-nav li',function(){
// 		console.log(1111);
// 	var bankname = $(this).text();
...
// 7查询银行卡全部记录
var pagesize = 50;
function bankrecord(page, bankcard, shuaixuan, start_date, end_date) {
	$.ajax({
			...
			if(data.status){
				...
				if(data.count != 0){
					...
					$.each(data.record_list, function (i, item) {
                            ...
                            if (item.type) {
                                //　出款
                                html += '<td>0</td><td>' + item.amount + '</td>';
                                ...
                                // 入款
                                html += '<td style="color: red;">' + item.amount + '</td><td style="color: red;">0</td>';
		...
		if(bankcard==''){
            ...
            if (amount && bankcard && now_time) {
                $.ajax({
                    ...
                    type: 'post',
                    dataType: 'json',
                    data: {'amount': amount, 'fees': fees, 'remark': remark, 'type': 'out', 'bankcard': bankcard, 'now_time': now_time},
                    success: function (data) {
                        if (data.status) {
                            console.log(data.msg)
                        } else {
			...
			$.ajax({
				...
				success: function (data) {
					if(data.status){
						...
						$('#in_amount_bankcard').html(data.msg.out_amount);
						$('#fees_bankcard').html(data.msg.fees);
						$('#total_in_amount_bankcard').html(data.msg.total_out_amount);
	...
	$.ajax({
		...
		success: function (data) {
			if(data.status){
				...
				$('#all_past_balance_bankcard').html(data.msg.past_balance);
				$('#all_in_amount_bankcard').html(data.msg.out_amount);
				$('#all_fees_bankcard').html(data.msg.fees);
				$('#all_total_in_amount_bankcard').html(data.msg.total_out_amount);
				$('#all_now_balance_bankcard').html(data.msg.now_balance);
				$('#zhong_past_balance_bankcard').html(data.msg.zhong_past_balance);
				$('#zhong_in_amount_bankcard').html(data.msg.zhong_out_amount);
				$('#zhong_fees_bankcard').html(data.msg.zhong_fees);
				$('#zhong_total_in_amount_bankcard').html(data.msg.zhong_total_out_amount);
